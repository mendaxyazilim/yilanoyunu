class NeuralNet {
  ...
  void show(float x, float y, float w, float h, float[] vision, float[] decision) {
     ...
     textSize(13);
     textAlign(CENTER,CENTER);
     text("Up",x+(lc*nSize)+(lc*nSpace)+nSize/2,y+oBuff+(nSize/2));
     text("Down",x+(lc*nSize)+(lc*nSpace)+nSize/2,y+oBuff+space+nSize+(nSize/2));
     text("Left",x+(lc*nSize)+(lc*nSpace)+nSize/2,y+oBuff+(2*space)+(2*nSize)+(nSize/2));
     text("Right",x+(lc*nSize)+(lc*nSpace)+nSize/2,y+oBuff+(3*space)+(3*nSize)+(nSize/2));
  }
}