...
void setup() {
  ...
  graphButton = new Button(349,15,100,30,"About AIX");
...
void draw() {
  ...
  if(humanPlaying) {
    ...
    if(!modelLoaded) {
      ...
      textSize(25);
      textAlign(LEFT);
      text("GENERATION : "+pop.gen,120,55);
      //text("BEST FITNESS : "+pop.bestFitness,120,50);
      //text("MOVES LEFT : "+pop.bestSnake.lifeLeft,120,70);
      text("MUTATION RATIO : "+mutationRate*100+"%",120,80);
      text("SCORE : "+pop.bestSnake.score,120,105);
      text("HIGHSCORE : "+highscore,120,130);
      increaseMut.show();
      decreaseMut.show();
    } else {
      model.look();
